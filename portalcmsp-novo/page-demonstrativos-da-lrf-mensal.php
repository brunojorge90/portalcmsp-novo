...
jQuery(document).ready(function($){
	...
	var elems = $('.alphalist2023 li').detach().sort(function (a, b) {
  return ($(a).text() < $(b).text() ? -1 
        : $(a).text() > $(b).text() ? 1 : 0);
}); 

var elems2 = $('.alphalist2024 li').detach().sort(function (a, b) {
  return ($(a).text() < $(b).text() ? -1 
        : $(a).text() > $(b).text() ? 1 : 0);
}); 
$('.alphalist2024').append(elems2);
$('.alphalist2023').append(elems);
				...
				<div id="inner-content" class="wrap cf">
						...
						<div id="main" class="two-column-main cf" role="main">
							...
							<article id="post-<?php the_ID(); ?>" <?php post_class( 'cf' ); ?> role="article" itemscope itemtype="http://schema.org/BlogPosting">
								...
								<h3>
									Consulte abaixo os demonstrativos fiscais mensais da Câmara Municipal de São Paulo:
								</h3>
								<?php
						...
						foreach ($posts as $post) { 
							...
							if(get_field("ano") < 2020 || get_field("ano") == 2021)
									continue;
					  ...
					  <?php }
					   ...
					   <?php }
								...
								if ( (stripos($titulo,'bimestre') != true) && (stripos($titulo,'Quadr') != true)
							//&& ($ano != "2004")  && ($ano != "2003")
							){
								?>
