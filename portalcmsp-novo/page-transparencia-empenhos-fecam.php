...
<div id="content">
	...
	<table id="grid" class="display dataTable" width="100%" cellspacing="0" style="font-size: .9em;" border="1">
		<thead>
		<tr>
			 <th>Empenho</th>
			 <th>Dotação</th>
			 <th>Credor/ fornecedor/ contratado</th>
			 <th>Data</th>
			 <th>Valor Total Empenhado</th>
			 <th>Valor Anulado</th>
			 <th>Valor Empenhado Líquido</th>
	...
	?>
		<tr>
			 <td data-order="<?php echo $dataOrder; ?>"> <?php echo "<a href='/empenho/?".$params."'>".$empenho->codEmpenho."/".$empenho->anoEmpenho."</a> "; ?></td>
             <td><?php echo $dotacao; ?> </td>
             <td> <?php echo $empenho->txtRazaoSocial ; ?> </td>
             <td> <?php echo date("d/m/Y", strtotime($empenho->datEmpenho)); ?> </td>
             <td><?php echo "<span style='display:none'>".str_pad(number_format($empenho->valTotalEmpenhado, 2, ',', '.'), 14, '0', STR_PAD_LEFT)."</span>".numfmt_format_currency($fmt, $empenho->valTotalEmpenhado, "BRL"); ?> </td>
             <td> <?php echo "<span style='display:none'>".str_pad(number_format($empenho->valAnuladoEmpenho, 2, ',', '.'), 14, '0', STR_PAD_LEFT)."</span>".numfmt_format_currency($fmt, $empenho->valAnuladoEmpenho, "BRL"); ?> </td>
             <td> <?php echo "<span style='display:none'>".str_pad(number_format($empenho->valEmpenhadoLiquido, 2, ',', '.'), 14, '0', STR_PAD_LEFT)."</span>".numfmt_format_currency($fmt, $empenho->valEmpenhadoLiquido, "BRL"); ?> </td>
             <td><?php echo "<span style='display:none'>".str_pad(number_format($empenho->valLiquidado, 2, ',', '.'), 14, '0', STR_PAD_LEFT)."</span>".numfmt_format_currency($fmt, $empenho->valLiquidado, "BRL"); ?> </td>			 
		</tr>     
<?php 
...
<form action="/wp-content/uploads/transparencia/22_empenhos.csv" method="post">
