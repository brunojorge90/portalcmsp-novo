...
<?php
	...
	//$contratos = $wpdb->get_results( "SELECT * FROM transp_contratos where codEmpresa=18 order by anoContrato desc, codContrato desc" );
	$contratos = $wpdb->get_results( "SELECT c.*, e.txtRazaoSocial FROM transp_contratos c, transp_empenhos e 
	where c.codEmpresa=18 and c.anocontrato=e.anocontrato and e.numContrato=c.codContrato 
...
<div>
	<table id="grid" class="display dataTable" width="100%" cellspacing="0" style="font-size: .9em;border-color: #f3f3f3;" border="1">
		...
		<tr>
			 <th>Cód. Contrato</th>
 			 <th>Número Original do Contrato</th>
	...
	<?php foreach ($contratos as $contrato){ 
			$params="codEmpresa=18&codContrato=".$contrato->codContrato."&anoContrato=".$contrato->anoContrato;	
	...
	?>
		...
		<!--data-order="<php echo $dataOrder; ?>"-->
			 <td style="padding:0px;"> <?php echo "<a href='/contrato/?".$params."'>".$contrato->codContrato."/".$contrato->anoContrato."</a> "; ?></td>
			 <td> <?php echo $contrato->numOriginalContrato; ?> </td>
...
<form action="wp-content/uploads/transparencia/18_contratos.csv" method="post">
